...
<x-app-layout>
    ...
    <div class="info-profile row justify-content-center text-white">
        <div class="col-10 col-md-8 col-lg-7 p-0 pt-3 rounded-3" style="height: 500px">
            <div class="background"></div>
            <img src="" alt="">
            <form id="profile-picture-form" method="POST" action="{{route('user.update', $user->id)}}" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="flex justify-content-between">
                    <label for="new-profile-image">
                        <div class="profile-image" style="cursor: {{auth()->user()->id == $user->id ? 'pointer' : 'auto'}}; background-image: url('{{ $user->profile_picture ? asset('storage/' . $user->profile_picture) : 'https://upload.wikimedia.org/wikipedia/commons/thumb/1/12/User_icon_2.svg/1200px-User_icon_2.svg.png' }}');"></div>
                        ...
                        <i class="fa-solid fa-user-pen"></i>
                        @endif
                        <input class="d-none" type="file" name="image" id="new-profile-image" onchange="localStorage.setItem('scrollPosition', 0); document.getElementById('profile-picture-form').submit();">
                        @endif
                    </label>
                </div>
            </form>
        </div>
        <div class="col-10 col-md-8 col-lg-7 nformation p-5 flex" style="">
            <div style="flex: 2;">
                <h1 class="flex-1 fs-1 mb-3">{{$user->name}}</h1>
            ...
            @if (Auth::id() != $user->id)
                <form action="{{ route('connection.store') }}" method="POST">
                    ...
                    @endif
                    @endif
                    >{{$connection->count() == 0 ? 'Connect' : ($connection[0]->status != 'accepted' ? 'Pending' : '')}}</button>
                </form>
            ...
            <div class="flex-1 flex justify-content-center align-items-center">
                <a href="{{route('profile.edit')}}" class="hover:text-gray-500"><i class="fa-solid fa-pen-to-square fs-2"></i></a>
            </div>
            @endif
        </div>
    </div>
        ...
        <div class="col-10 col-md-8 col-lg-7 col-xl-6 mx-xl-5 p-3 rounded-3">
            <form method="POST" action="{{route('post.store')}}" enctype="multipart/form-data">
                ...
                <div class="flex mb-3 justify-content-between">
                    <a href="{{route('user.show', $user->id)}}"><img width="80px" height="80" src="https://upload.wikimedia.org/wikipedia/commons/thumb/1/12/User_icon_2.svg/1200px-User_icon_2.svg.png" alt=""></a>
